...
namespace RaspberryModules.App.Modules
{
    public class SPIDisplay
    {
        ...
        public void WriteToScreen(String TheText)
        {
            List<string> lstText = new List<string>();
            int counter = 0;
            while (counter < 80 && (counter%20 == 0 || counter == 0))
            {
                int TextLen = TheText.Length - counter;
                int GetText = TextLen > 20 ? 20 : TextLen;
                lstText.Add(TheText.Substring(counter, GetText).Trim());
                counter += GetText;
                if (TextLen < 20)
                    counter = 100;
            }

            WriteLinesToScreen(lstText);
        }
